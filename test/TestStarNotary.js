...
it('lets 2 users exchange stars', async() => {
    ...
    let OwnerStar1Befor = await instance.ownerOf(starId1);
    let OwnerStar2Befor = await instance.ownerOf(starId2);
    let xch = await instance.exchangeStars(starId1, starId2,{from: user1});
    let OwnerStar1After = await instance.ownerOf(starId1);
    let OwnerStar2After = await instance.ownerOf(starId2);
    assert.equal(OwnerStar1Befor, OwnerStar2After);
    assert.equal(OwnerStar2Befor, OwnerStar1After);
});
...
it('lets a user transfer a star', async() => {
    ...
    let OwnerStar1After = await instance.ownerOf(starId1);
    assert.equal(OwnerStar1After, user2);
});
